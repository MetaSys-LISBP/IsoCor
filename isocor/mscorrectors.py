...
class HighResMSMSMetaboliteCorrector(HighResMetaboliteCorrector):
    """Metabolite *corrector* for high-resolution MS/MS data.
    ...
    def __init__(self, formula, tracer, resolution, mz_of_resolution, resolution_formula_code, charge, fragmentation_pattern, **kwargs):
        LowResMetaboliteCorrector.__init__(self, formula, tracer, charge=charge, **kwargs)
        # Some checks on the inputs
        try:
            resolution = float(resolution)
        ...
        if self.correction_limit >= 0.5:
            ...
            self._correction_limit = precision_machine
        # Log if direct instantiation
        if self.__class__.__name__ == HighResMSMSMetaboliteCorrector.__name__:
            self._log_on_init()
